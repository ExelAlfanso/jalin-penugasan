...
const MAGIC_REGEXP_SPECIFIERS = /* @__PURE__ */ new Set(["magic-regexp", "magic-regexp/further-magic"]);
function getParserLang(id) {
	const pathname = id.split("?")[0];
	if (pathname.endsWith(".tsx")) return "tsx";
	if (pathname.endsWith(".jsx")) return "jsx";
	if (/\.(?:[cm]?ts|vue)$/.test(pathname)) return "ts";
	return "js";
}
const MagicRegExpTransformPlugin = createUnplugin(() => {
	return {
		...
		transformInclude(id) {
			...
			if (/\.[cm]?[jt]sx?$/.test(pathname)) return true;
			return false;
		},
		transform: {
			filter: { code: "magic-regexp" },
			handler(code, id) {
				if (!code.includes("magic-regexp")) return;
				let ast;
				try {
					ast = this.parse(code, { lang: getParserLang(id) });
				} catch {
					ast = this.parse(code);
				}
