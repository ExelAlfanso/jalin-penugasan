import { n as deepCopy, r as loadKit, t as applyEnv } from "./utils-whMnUpQ0.mjs";
...
async function startNuxtAndGetViteConfig(rootDir = process.cwd(), options = {}) {
	...
	if (!nuxt.options._installedModules.find((i) => i?.meta?.name === "@nuxt/test-utils")) throw new Error("Failed to load `@nuxt/test-utils/module`. You may need to add it to your nuxt.config.");
...
async function defineVitestProject(config) {
	const resolvedConfig = await resolveConfig(defu$1({ test: { environment: "nuxt" } }, config));
...
function defineVitestConfig(config = {}) {
	return defineViteConfig(async () => {
		...
		if (defaultEnvironment !== "nuxt") {
			...
			nuxtProject.test.include = ["**/*.nuxt.{test,spec}.{js,mjs,cjs,ts,mts,cts,jsx,tsx}", "{test,tests}/nuxt/**/*.{test,spec}.{js,mjs,cjs,ts,mts,cts,jsx,tsx}"];
