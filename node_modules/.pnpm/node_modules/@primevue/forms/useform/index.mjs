...
// Split a dotted/bracketed path ("a.b", "a[0].b") into its canonical tokens (["a","0","b"]). Single source
// for the notation-agnostic split used by groupKeys / getValueByPath / findFieldErrors.
...
function groupKeys(obj) {
  return Object.entries(obj).reduce(function (result, _ref) {
    var _ref2 = _slicedToArray(_ref, 2),
      key = _ref2[0],
      value = _ref2[1];
    toPathTokens(key).reduce(function (acc, curr, idx, arr) {
      var _acc$curr;
...
function getValueByPath(obj, path) {
  ...
  var keys = toPathTokens(path);
  return keys.reduce(function (acc, key) {
    return acc && acc[key] !== undefined ? acc[key] : undefined;
  }, obj);
}
function normalizePath(path) {
  return toPathTokens(path).join('.');
}
function findFieldErrors(errors, fieldName) {
  // Resolvers key errors by a path string but differ on array-index notation — yup emits "emails[0].title"
  // (bracket) while zod emits "emails.0.title" (dot). They denote the same path, so try the regular lookups
  // first (fast path + nested traversal), then fall back to matching any flat key whose canonical token
  // path equals the field's — making error matching resolver-agnostic without touching field names.
  var direct = getValueByPath(errors, fieldName);
  if (isNotEmpty(direct)) return direct;
  var target = normalizePath(fieldName);
  for (var key in errors) {
    if (normalizePath(key) === target) return errors[key];
  }
  return undefined;
}
var useForm = function useForm() {
  ...
  // Flipped true once the mount-time validation pass has run (see `validateOnMounted`). A field that
  // registers after this — e.g. a `v-if` FormField becoming visible — missed that pass, so `defineField`
  // re-applies `validateOnMount` for just that field.
  var mounted = false;
  ...
  var validateOn = /*#__PURE__*/function () {
    var _validateOn = _asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee(option, defaultValue) {
      var _options$option;
    ...
    function validateOn(_x, _x2) {
      return _validateOn.apply(this, arguments);
    }
    return validateOn;
  }();
  ...
  var defineField = function defineField(field, fieldOptions) {
    ...
    // If the form already mounted, a field appearing now (e.g. a conditionally-rendered FormField) missed
    // the mount-time validation pass — validate it when validateOnMount applies (field-level opt-in, or
    // form-level `true`/name-list) so a newly-visible required field shows invalid immediately instead of
    // reading as valid. `validateFieldOn` can't be reused here: FormField always passes an explicit
    // `validateOnMount: false`, whose `??` short-circuit would mask the form-level setting.
    if (mounted && (fieldOptions !== null && fieldOptions !== void 0 && fieldOptions.validateOnMount || isFieldValidate(field, options.validateOnMount))) {
      validate(field);
    }
  ...
  var handleReset = function handleReset(callback) {
    return /*#__PURE__*/function () {
      var _ref4 = _asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee3(event) {
  ...
  var validate = /*#__PURE__*/function () {
    var _validate = _asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee4(field) {
      var _yield$options$resolv, _options$resolver, _result, _result$errors, _result$values;
      var resolverOptions, _ref7, names, values, result, flattenFields, _i, _Object$entries, _Object$entries$_i, fieldName, fieldInst, recheck, _fieldInst$options, _findFieldErrors, _errors$2, fieldResolver, _yield$fieldResolver, fieldValue, fieldResult, errors, _t3, _t4, _t5, _t6, _t7, _t8;
      return _regenerator().w(function (_context4) {
        while (1) switch (_context4.n) {
          case 0:
            resolverOptions = Object.entries(_states).reduce(function (acc, _ref5) {
            ...
            _ref7 = [resolverOptions.names, groupKeys(resolverOptions.values)], names = _ref7[0], values = _ref7[1];
            _context4.n = 1;
          ...
          case 5:
            ...
            // Interaction gate: keeps single-field validation from leaking into pristine fields.
            recheck = isEmpty(result.errors) && (fieldInst.states.touched || fieldInst.states.dirty || fieldInst.states.invalid);
            if (!(flattenFields.includes(fieldName) || !field || recheck)) {
              _context4.n = 11;
              break;
            }
          ...
          case 10:
            errors = (_findFieldErrors = findFieldErrors(result.errors, fieldName)) !== null && _findFieldErrors !== void 0 ? _findFieldErrors : []; //const value = result.values?.[fieldName] ?? _states[sField].value;
          ...
          case 12:
            return _context4.a(2, _objectSpread(_objectSpread({}, result), {}, {
              // Guarantee `values` is always present: a custom resolver may return `{ errors }`
              // without echoing `values` back, but FormSubmitEvent.values is documented as always set.
              values: (_result$values = result.values) !== null && _result$values !== void 0 ? _result$values : values,
  ...
  var reset = function reset() {
    Object.keys(_states).forEach(/*#__PURE__*/function () {
      var _ref8 = _asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee5(field) {
        var _fields$field3;
        var watcher;
  ...
  var unregister = function unregister(field) {
    var _fields$field$_watche;
    if (!field || !fields[field]) return;

    // Stop the value watcher and drop the field so it no longer leaks into states/values/valid.
    // Needed when a field is removed from the UI (e.g. deleting an item from an array of objects).
    (_fields$field$_watche = fields[field]._watcher) === null || _fields$field$_watche === void 0 || _fields$field$_watche.stop();
    delete fields[field];
