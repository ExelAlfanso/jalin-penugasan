...
function registerStyles(resolvePath, registered, moduleOptions) {
  ...
  if (!options?.unstyled) {
    ...
    [registered.components, registered.directives].flat().reduce((acc, citem) => acc.some((item) => item.as.toLowerCase() === citem.as.toLowerCase()) ? acc : [...acc, citem], []).forEach((item) => {
      styles.push({
        name: `${item.as}Style`,
        as: `${item.as}Style`,
...
const module = defineNuxtModule({
  ...
  setup(moduleOptions, nuxt) {
    ...
    if (hasTheme) {
      nuxt.options.build.transpile.push("@primeuix/themes");
    }
    const registeredStyles = registered.styles;
    if (autoImport) {
...
export default defineNuxtPlugin(({ vueApp }) => {
  ...
  const license = runtimeConfig?.public?.PRIMEUI_LICENSE || options?.license || undefined;
  const pt = ${importPT ? `{ pt: ${importPT.as} }` : `{}`};
  const theme = ${hasTheme ? `{ theme: ${importTheme?.as} || options?.theme }` : `{}`};

  usePrimeVue && vueApp.use(PrimeVue, { ...options, ...pt, ...theme, license });
