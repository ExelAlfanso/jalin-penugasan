import { C as getRegistryItems, bt as RegistryError, d as getRegistriesConfig, t as searchRegistries } from "./registry-hxzbzXMt.js";
	...
	return { tools: [
		{
			...
			inputSchema: zodToJsonSchema(z$1.object({}))
		},
		{
			name: "list_items_in_registries",
			description: "List items from registries (requires components.json - use init_project if missing)",
			inputSchema: zodToJsonSchema(z$1.object({
				registries: z$1.array(z$1.string()).describe("Array of registry names to search (e.g., ['@shadcn', '@acme'])"),
				limit: z$1.number().optional().describe("Maximum number of items to return"),
				offset: z$1.number().optional().describe("Number of items to skip for pagination")
			}))
		},
		{
			...
			inputSchema: zodToJsonSchema(z$1.object({
				registries: z$1.array(z$1.string()).describe("Array of registry names to search (e.g., ['@shadcn', '@acme'])"),
				query: z$1.string().describe("Search query string for fuzzy matching against item names and descriptions"),
				limit: z$1.number().optional().describe("Maximum number of items to return"),
				offset: z$1.number().optional().describe("Number of items to skip for pagination")
			}))
		},
		{
			...
			inputSchema: zodToJsonSchema(z$1.object({ items: z$1.array(z$1.string()).describe("Array of item names with registry prefix (e.g., ['@shadcn/button', '@shadcn/card'])") }))
		},
		{
			...
			inputSchema: zodToJsonSchema(z$1.object({
				registries: z$1.array(z$1.string()).describe("Array of registry names to search (e.g., ['@shadcn', '@acme'])"),
				query: z$1.string().describe("Search query for examples (e.g., 'accordion-demo', 'button demo', 'card example', 'tooltip-demo', 'example-booking-form', 'example-hero'). Common patterns: '{item-name}-demo', '{item-name} example', 'example {item-name}'")
			}))
		},
		{
			...
			inputSchema: zodToJsonSchema(z$1.object({ items: z$1.array(z$1.string()).describe("Array of items to get the add command for prefixed with the registry name (e.g., ['@shadcn/button', '@shadcn/card'])") }))
		},
		{
	...
	try {
		...
		switch (request.params.name) {
			...
			case "search_items_in_registries": {
				const args = z$1.object({
					registries: z$1.array(z$1.string()),
					query: z$1.string(),
			...
			case "view_items_in_registries": {
				const args = z$1.object({ items: z$1.array(z$1.string()) }).parse(request.params.arguments);
			...
			case "get_add_command_for_items": return { content: [{
				type: "text",
				text: await npxShadcnVue(`add ${z$1.object({ items: z$1.array(z$1.string()) }).parse(request.params.arguments).items.join(" ")}`)
			}] };
		...
		if (error instanceof z$1.ZodError) return {
			content: [{
				type: "text",
...
//# sourceMappingURL=mcp-C5R50TEt.js.map