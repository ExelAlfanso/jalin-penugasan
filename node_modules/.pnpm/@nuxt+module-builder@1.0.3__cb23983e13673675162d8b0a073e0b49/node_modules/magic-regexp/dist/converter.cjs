...
let regexp_tree = require("regexp-tree");
regexp_tree = require_rolldown_runtime.__toESM(regexp_tree);
...
function build(node) {
	...
	switch (node.type) {
		case "CharacterClass": {
			...
			if (exprs.length === 1) {
				const first = exprs[0];
				if (typeof first === "string") return node.negative ? `charNotIn(${first})` : `charIn(${first})`;
					...
					if (range === "A-Z") return node.negative ? `not.letter.uppercase` : `letter.uppercase`;
					else if (range === "a-z") return node.negative ? `not.letter.lowercase` : `letter.lowercase`;
				}
				...
				if (typeof exprs[0] !== "string" && typeof exprs[1] !== "string") {
					...
					if (range1 === "A-Z" && range2 === "a-z" || range1 === "a-z" && range2 === "A-Z") return node.negative ? `not.letter` : `letter`;
				}
			}
		...
		case "Assertion": switch (node.kind) {
			case "\\b": return "wordBoundary";
			case "\\B": return "not.wordBoundary";
			case "^": return chain("", "at.lineStart()");
			case "$": return chain("", "at.lineEnd()");
			case "Lookbehind": return chain("", `${node.negative ? "notAfter" : "after"}(${build(node.assertion)})`);
			case "Lookahead": return chain("", `${node.negative ? "notBefore" : "before"}(${build(node.assertion)})`);
			/* v8 ignore next 2 */
			default: throw new TypeError(`Unknown Assertion kind: ${node.kind}`);
		}
		case "Char": if (node.kind === "meta") switch (node.value) {
			case ".": return "char";
			case "\\w": return "wordChar";
			case "\\d": return "digit";
			case "\\s": return "whitespace";
			case "\\t": return "tab";
			case "\\n": return "linefeed";
			case "\\r": return "carriageReturn";
			case "\\W": return "not.wordChar";
			case "\\D": return "not.digit";
			case "\\S": return "not.whitespace";
			default: throw new Error(`Unsupported Meta Char: ${node.value}`);
		}
		else {
		...
		case "Repetition": {
			...
			if (!quantifier.greedy) throw new Error("Unsupported for lazy quantifier");
			switch (quantifier.kind) {
				case "+": return `oneOrMore(${expr})`;
				case "?": return `maybe(${expr})`;
				case "*": return chain(expr, "times.any()");
				case "Range":
					if (quantifier.from === quantifier.to) return chain(expr, `times(${quantifier.from})`);
					else if (!quantifier.to) return chain(expr, `times.atLeast(${quantifier.from})`);
					else if (quantifier.from === 0) return chain(expr, `times.atMost(${quantifier.to})`);
		...
		case "Alternative": {
			...
			for (let i = 0; i < alts.length; i++) {
				...
				if (alt.type === "Assertion") switch (alt.kind) {
					case "^": {
						const next = alts[++i];
						if (next === void 0) throw new Error(`Unexpected assertion: ${JSON.stringify(alt)}`);
						exprs.push(chain(next, "at.lineStart()"));
						continue;
					}
					case "$": {
						const prev = exprs.pop();
						if (prev === void 0) throw new Error(`Unexpected assertion: ${JSON.stringify(alt)}`);
		...
		case "Group": if (node.capturing) return chain(build(node.expression), node.name ? `as('${node.name}')` : "grouped()");
		else return chain(build(node.expression));
		/* v8 ignore next 2 */
		case "Backreference": return chain("", `and.referenceTo('${node.reference}')`);
	}
}
...
function combineContinuousSimpleChars(expressions) {
	let simpleChars = "";
	const exprs = expressions.reduce((acc, expr) => {
		const char = expr.type === "Char" ? getChar(expr) : null;
		if (char !== null) simpleChars += char;
		else {
			if (simpleChars) {
	...
	if (simpleChars) exprs.push(`'${simpleChars}'`);
	return exprs;
}
function getChar(char) {
	function escapeSimpleChar(char) {
		return char === "'" ? "\\'" : char;
	}
	switch (char.kind) {
		case "simple": return escapeSimpleChar(char.value);
		case "oct":
		case "decimal":
		case "hex":
		case "unicode": if ("symbol" in char) return escapeSimpleChar(char.symbol);
	}
	return null;
}
/** Whether `expr` is a list of arguments rather than a single chainable expression. */
function isArgumentList(expr) {
	let depth = 0;
	let quoted = false;
...
function chain(expr, helper) {
	...
	let _expr = value;
	if (value === "") _expr = "exactly('')";
	else if (helper && isArgumentList(value)) _expr = `exactly(${value})`;
	else if (value.startsWith("'") && value.endsWith("'")) _expr = `exactly(${value})`;
...
function convert(regex, { argsOnly = false } = {}) {
	const ast = regexp_tree.default.parse(regex);
