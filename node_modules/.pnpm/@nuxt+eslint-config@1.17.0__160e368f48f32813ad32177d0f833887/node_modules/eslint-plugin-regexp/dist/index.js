...
function getUsageOfPattern(node, context) {
	...
	if (usageSet.has(0)) return usageSet.has(1) ? 2 : 0;
	return usageSet.has(1) ? 1 : 3;
}
...
function* iterateUsageOfPattern(node, context) {
	for (const ref of extractExpressionReferences(node, context)) if (ref.type === "member") yield* iterateUsageOfPatternForMemberExpression(ref.memberExpression, context);
		...
		if (propName === "match" || propName === "matchAll" || propName === "split" || propName === "replace" || propName === "replaceAll" || propName === "search") yield 1;
		else yield 3;
	} else yield 3;
	else yield 3;
}
...
*/
const RESERVED_DOUBLE_PUNCTUATOR_CHARS = /* @__PURE__ */ new Set(RESERVED_DOUBLE_PUNCTUATORS);
/**
...
function getEscapeSequenceKind(raw) {
	...
	if (isOctalEscape(raw)) return "octal";
	if (isControlEscape(raw)) return "control";
	if (isHexadecimalEscape(raw)) return "hexadecimal";
	if (isUnicodeEscape(raw)) return "unicode";
	if (isUnicodeCodePointEscape(raw)) return "unicode code point";
	return null;
}
/**
* Returns whether the given raw of a character literal is an octal escape
...
function isHexLikeEscape(raw) {
	...
	return kind === "hexadecimal" || kind === "unicode" || kind === "unicode code point";
}
...
function jsDocTypeNodeToTypeInfo(node) {
	...
	if (node.type === "JsdocTypeBigInt") return BIGINT;
	...
	if (node.type === "JsdocTypeImport" || node.type === "JsdocTypeKeyof" || node.type === "JsdocTypeNamePath" || node.type === "JsdocTypePredicate" || node.type === "JsdocTypeSpecialNamePath" || node.type === "JsdocTypeSymbol" || node.type === "JsdocTypeUniqueSymbol" || node.type === "JsdocTypeAssertsPlain" || node.type === "JsdocTypeInfer") return null;
...
const CONTROL_CHARS$1 = /* @__PURE__ */ new Map([
	[0, "\\0"],
	[9, "\\t"],
	[10, "\\n"],
	[11, "\\v"],
...
var hexadecimal_escape_default = createRule("hexadecimal-escape", {
	...
	create(context) {
		...
		function verifyForAlways({ node, getRegexpLocation, fixReplaceNode }, kind, cNode) {
			if (kind !== "unicode" && kind !== "unicode code point") return;
		...
		function createVisitor(regexpContext) {
			...
			return {
				onCharacterEnter(cNode) {
					...
					if (escapeKind === "unicode" || escapeKind === "unicode code point") verifyCharacterInUnicodeEscape(regexpContext, cNode);
					if (escapeKind === "hexadecimal") verifyCharacterInHexadecimalEscape(regexpContext, cNode);
					if (escapeKind === "control") verifyCharacterInControl(regexpContext, cNode);
				},
...
const CONTROL_CHARS = /* @__PURE__ */ new Map([
	[0, "\\0"],
	[9, "\\t"],
	[10, "\\n"],
	[11, "\\v"],
...
function getSubsetRelation(left, right) {
	try {
		...
		if (subset && superset) return 1;
		else if (subset) return 2;
		else if (superset) return 3;
		return 0;
	} catch {
		return 4;
	}
...
function getPartialSubsetRelation(left, leftIsPartial, right, rightIsPartial) {
	...
	if (relation === 0 || relation === 4) return relation;
...
var no_dupe_disjunctions_default = createRule("no-dupe-disjunctions", {
	...
	create(context) {
		const reportExponentialBacktracking = context.options[0]?.reportExponentialBacktracking ?? "potential";
		const reportUnreachable = context.options[0]?.reportUnreachable ?? "certain";
		const report = context.options[0]?.report ?? "trivial";
		...
		function createVisitor(regexpContext) {
			...
			function getFilterInfo(parentNode) {
				...
				let stared;
				if (isStared(parentNode)) stared = 1;
				else if (usage === 0 || usage === 2) stared = 2;
				else stared = 0;
				let nothingAfter;
				if (!hasNothingAfterNode(parentNode)) nothingAfter = 0;
				else if (usage === 0 || usage === 2) nothingAfter = 2;
				else nothingAfter = 1;
				let reportExp;
			...
			function filterResults(results, { nothingAfter, reportExp, reportPrefix }) {
				switch (report) {
					...
					case "trivial": return results.filter(({ type }) => {
						switch (type) {
							case "Duplicate":
							case "Subset":
					...
					case "interesting": return results.filter(({ type }) => {
						switch (type) {
							case "Duplicate":
							case "Subset":
							case "NestedSubset": return true;
							case "Overlap": return reportExp;
							case "Superset": return reportExp || nothingAfter === 0;
							case "PrefixSubset":
							case "PrefixNestedSubset": return reportPrefix;
							default: throw assertNever(type);
						}
			...
			function reportResult(result, { stared }) {
				let exp;
				if (stared === 1) exp = " This ambiguity is likely to cause exponential backtracking.";
				else if (stared === 2) exp = " This ambiguity might cause exponential backtracking.";
				else exp = "";
	...
	create(context) {
		...
		function createVisitor(regexpContext) {
			...
			const result = analyse(getJSRegexppAst(regexpContext), {
				...
				assumeRejectingSuffix: reportUncertain && getUsageOfPattern() !== 1
			});
			for (const report of result.reports) {
				const exp = report.exponential ? " This is going to cause exponential backtracking resulting in exponential worst-case runtime behavior." : getUsageOfPattern() !== 1 ? " This might cause exponential backtracking." : "";
	...
	create(context) {
		...
		*/
		...
		function createVisitor(regexpContext) {
			...
			const usage = getUsageOfPattern();
			if (ignorePartial && usage === 0) return {};
			const assumeRejectingSuffix = reportUncertain && usage !== 1;
...
const ESCAPES_OUTSIDE_CHARACTER_CLASS_WITH_U = /* @__PURE__ */ new Set([...ESCAPES_OUTSIDE_CHARACTER_CLASS, "}"]);
...
const REGEX_CHAR_CLASS_ESCAPES = /* @__PURE__ */ new Set([
	CP_BACK_SLASH,
	CP_CLOSING_BRACKET,
	CP_MINUS
]);
const REGEX_CLASS_SET_CHAR_CLASS_ESCAPE = /* @__PURE__ */ new Set([
	CP_BACK_SLASH,
	CP_SLASH,
	CP_OPENING_BRACKET,
	CP_CLOSING_BRACKET,
...
const REGEX_ESCAPES = /* @__PURE__ */ new Set([
	CP_BACK_SLASH,
	CP_SLASH,
	CP_CARET,
	CP_DOT,
...
const POTENTIAL_ESCAPE_SEQUENCE_FOR_CHAR_CLASS = /* @__PURE__ */ new Set([...POTENTIAL_ESCAPE_SEQUENCE, "q"]);
	...
	*/
	function reportUselessGlobalFlag(regExpReference, data) {
		...
		context.report({
			...
			messageId: data.kind === 0 ? "uselessGlobalFlagForSplit" : data.kind === 1 ? "uselessGlobalFlagForSearch" : data.kind === 3 ? "uselessGlobalFlagForTest" : data.kind === 2 ? "uselessGlobalFlagForExec" : "uselessGlobalFlag",
...
var no_useless_non_capturing_group_default = createRule("no-useless-non-capturing-group", {
	...
	create(context) {
		...
		function createVisitor({ node, getRegexpLocation, fixReplaceNode, getUsageOfPattern }) {
			...
			else if (allowTop === "partial") if (getUsageOfPattern() !== 1) isIgnored = isTopLevel;
			...
			return { onGroupEnter(gNode) {
				...
				context.report({
					...
					fix: fixReplaceNode(gNode, () => {
						if (allowTop === "never" && isTopLevel(gNode) && getUsageOfPattern() !== 1) return null;
...
var optimal_quantifier_concatenation_default = createRule("optimal-quantifier-concatenation", {
	...
	create(context) {
		const cgReporting = context.options[0]?.capturingGroups ?? "report";
		function createVisitor(regexpContext) {
			const { node, flags, getRegexpLocation, fixReplaceNode } = regexpContext;
			const parser = getParser(regexpContext);
			...
			return {
				onQuantifierEnter(quantifier) {
					...
					if (involvesCapturingGroup && cgReporting === "ignore") return;
				...
				onAlternativeLeave(aNode) {
					for (let i = 0; i < aNode.elements.length - 1; i++) {
						...
						if (involvesCapturingGroup && cgReporting === "ignore") continue;
...
function getSideEffectsWhenReplacingCapturingGroup(elements, start, end, { flags }) {
	...
	if (start) {
		...
		if (!hasDisjoint(chars, elements.slice(1))) result.add(0);
		else {
			const last = elements[elements.length - 1];
			if (!FirstConsumedChars.toLook(getFirstConsumedCharPlusAfter(last, "rtl", flags)).char.isDisjointWith(chars)) result.add(0);
		}
	}
	if (end && flags.global) {
		...
		if (first) {
			...
			if (!FirstConsumedChars.toLook(getFirstConsumedCharPlusAfter(first, "ltr", flags)).char.isDisjointWith(chars)) result.add(1);
		}
	}
	return result;
		...
		function createVerifyVisitor(regexpContext, parsedElements, replaceReferenceList) {
			...
			return {
				...
				onPatternLeave() {
					...
					if (sideEffects.has(0)) reportStart = null;
					if (sideEffects.has(1)) reportEnd = null;
		...
		function createVisitor({ node, flags, getRegexpLocation, fixReplaceNode, patternSource }) {
			return { onCharacterClassEnter(ccNode) {
				...
				if (lowerAToZ.length && capitalAToZ.length && digit.length && underscore.length) {
					const unexpectedElements = [.../* @__PURE__ */ new Set([
						...lowerAToZ,
						...capitalAToZ,
						...digit,
						...underscore
...
var unicode_escape_default = createRule("unicode-escape", {
	...
	create(context) {
		...
		function verifyForUnicodeEscape({ node, getRegexpLocation, fixReplaceNode }, kind, cNode) {
			if (kind !== "unicode code point") return;
